...
class DeliveryCarrier(models.Model):
    ...
    country_groups = fields.Many2many(
        "res.country.group",
        string="Automatically send paperless invoice",
    ...
    def ups_rate_shipment(self, order):
        ...
        try:
            ...
            # During rate shopping (checkout), return failure instead of raising exception
            ...
            # 111217: The requested service is unavailable between the selected locations
    ...
    def prepare_paperless_invoice_provider(self, picking):
        """prepare the paperless document dictionary for up request"""
        paperless_document_data = []
        for file_data in picking.ups_paperless_document:
            document_data = {
        ...
        if "002" not in picking.ups_paperless_document.mapped("ups_document_type"):
            ...
            invoice_pdf_file = (
                ...
                ._render_qweb_pdf(picking.sale_id.invoice_ids.ids)
            )
            ...
            paperless_document_data.append(document_data)
            paperless_document_obj = self.env["ups.paperless.document"].search(
                [("ups_stock_picking_id", "=", picking.id)]
            )
            paperless_document_obj.create(
                {
        ...
        if "010" not in picking.ups_paperless_document.mapped("ups_document_type"):
    ...
    def ups_paperless_invoice_provider(self, picking):
        ...
        # Get the list of paperless documents
        paperless_document_data = self.prepare_paperless_invoice_provider(picking)

        # Don't send if no documents
