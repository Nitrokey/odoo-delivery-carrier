...
class UpsRequest:
    ...
    def _prepare_create_shipping(self, picking):
        ...
        if picking.ups_document_id and picking.ups_document_id.strip():
            try:
                document_id = (
                    ast.literal_eval(picking.ups_document_id)
                    if len(picking.ups_document_id) > 30
                    else picking.ups_document_id
                )
            except (ValueError, SyntaxError):
                document_id = picking.ups_document_id
    ...
    def _send_shipping(self, picking):
        ...
        if picking.ups_paperless_auto_send and not picking.ups_document_id:
            try:
    ...
    def send_paperless_invoice(self, picking, paperless_document_data):
        ...
        try:
            ...
            if response.status_code in [200, 201]:
                ...
                picking.ups_document_id = document_id
                return document_id
            errors = (json.loads(response.text).get("response") or {}).get("errors")
